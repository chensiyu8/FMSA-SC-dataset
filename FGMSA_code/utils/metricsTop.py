...
class MetricsTop:
    def __init__(self, train_mode):
        if train_mode == "regression":
            self.metrics_dict = {
                'FMSA': self.__eval_fmsa_regression
            }
        else:
            self.metrics_dict = {
                'FMSA': self.__eval_fmsa_classification
            }

    def __eval_fmsa_classification(self, y_pred, y_true):
    ...
    def __eval_fmsa_regression(self, y_pred, y_true):
        test_preds = y_pred.view(-1).cpu().detach().numpy()
        test_truth = y_true.view(-1).cpu().detach().numpy()
